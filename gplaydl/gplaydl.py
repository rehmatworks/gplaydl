...
ap.add_argument("--device", dest="device", help="Device code name", default=devicecode)
...
cp.add_argument("--password", dest="password", help="Google password", default=None)

# Args for downloading an app
dl = subparsers.add_parser("download", help="Download an app or a game from Google Play.")
dl.add_argument("--packageId", dest="packageId", help="Package ID of the app, i.e. com.whatsapp")
dl.add_argument("--path", dest="storagepath", help="Path where to store downloaded files", default=False)
dl.add_argument("--ex", dest="expansionfiles", action="store_const", const=True, help="Download expansion (OBB) data if available", default=True)
...
def configureauth():
	...
	try:
		...
		print(colored("Configuration file created successfully! Try downloading an app now.", "green"))
	except:
...
def downloadapp(packageId, expansionFiles=True, storagepath="./"):
	...
	try:
		...
		for obb in download["additionalData"]:
			name = "%s.%s.%s.obb" % (obb["type"], str(obb["versionCode"]), download["docId"])
